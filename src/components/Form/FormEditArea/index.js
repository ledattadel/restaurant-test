...
const FormEditArea = ({ visible, setVisible, data }) => {
    ...
    const addArea = async (values) => {
        // values.companyId = 0;
        const api = await dispatch(actions.updatedAreas(data.id, values));
    };
    ...
    return (
        <div className="add-dish">
            <Title level={3} className="add-dish__title">
                Cập nhật khu vực bàn {data.id}
            </Title>
            <Form
                initialValues={{
                    ['name']: data.name,
                    ['status']: data.areaStatus.code,
